                    ...
                    {WEEKS_ALL.map((w) => {
                      ...
                      return (
                        <div key={`mid-hdr-${w}`} className="relative">
                          <div className="bg-white rounded-lg p-4 text-center border border-slate-200 shadow-sm min-h-[64px] flex flex-col justify-center">
                            <div className={`text-[11px] font-medium mb-2 ${isNA ? 'text-red-700' : 'text-emerald-700'}`}>
                              {formatUnits(availableUnits)}
                            </div>
                            <div className="w-full h-0.5 bg-gradient-to-r from-transparent via-teal-400 to-transparent mb-2"></div>
